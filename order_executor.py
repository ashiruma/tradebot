...
class OrderExecutor:
    ...
    def __init__(self, client: OKXClient, state_manager=None):
        self.client = client
        ...
        if state_manager:
            ...
            if self.pending_orders:
                print(f"[v0] Restored {len(self.pending_orders)} pending orders from state")
        
    def _save_state(self):
        """Save pending orders to state manager"""
    ...
    def recover_pending_orders(self):
        """Check status of all pending orders (for crash recovery)"""
        if not self.pending_orders:
            return
        
        print(f"[v0] Checking {len(self.pending_orders)} pending orders...")
